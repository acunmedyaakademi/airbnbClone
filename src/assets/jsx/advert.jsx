import { StarFilled,HeartTwoTone } from '@ant-design/icons';



export const createAdverts = () => {

    return(
        <>
            
            <div className="card">
                <div style={{ zIndex: "1", position: "absolute", marginLeft: "285px", marginTop: "10px" }}>
                    <HeartTwoTone twoToneColor="#eb2f96" style={{ fontSize: "22px" }} />
                </div>
                <img src="https://a0.muscache.com/im/pictures/6943ce46-c64f-43b4-824b-5551735befcc.jpg?im_w=720" alt="" />
                <div className="info">
                    <div className="title" style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
                        <p className="title" >
                            <strong>Bağcılar, Türkiye</strong>
                        </p>
                        <p><StarFilled style={ {fontSize:"12px"}} /> 4,4</p>
                    </div>
                    <h5 style={{color:"#868686"}}>Geçen hafta 204 kez görüntülendi</h5>
                    <h5 style={{color:"#868686"}}>7 - 14 OCAK</h5>
                    <h5 style={{color:"#1d1d1d"}}><strong>5500TL</strong> Gece</h5>
                </div>
            </div>
            <div className="card">
                <div style={{ zIndex: "1", position: "absolute", marginLeft: "285px", marginTop: "10px" }}>
                    <HeartTwoTone twoToneColor="#eb2f96" style={{position:"absolute", fontSize: "22px" }} />
                </div>
                <img src="https://a0.muscache.com/im/pictures/6943ce46-c64f-43b4-824b-5551735befcc.jpg?im_w=720" alt="" />
                <div className="info">
                    <div className="title" style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
                        <p className="title" >
                            <strong>Bağcılar, Türkiye</strong>
                        </p>
                        <p><StarFilled style={ {fontSize:"12px"}} /> 4,4</p>
                    </div>
                    <h5 style={{color:"#868686"}}>Geçen hafta 204 kez görüntülendi</h5>
                    <h5 style={{color:"#868686"}}>7 - 14 OCAK</h5>
                    <h5 style={{color:"#1d1d1d"}}><strong>5500TL</strong> Gece</h5>
                </div>
            </div>
            <div className="card">
                <div style={{ zIndex: "1", position: "absolute", marginLeft: "285px", marginTop: "10px" }}>
                    <HeartTwoTone twoToneColor="#eb2f96" style={{ fontSize: "22px" }} />
                </div>
                <img src="https://a0.muscache.com/im/pictures/6943ce46-c64f-43b4-824b-5551735befcc.jpg?im_w=720" alt="" />
                <div className="info">
                    <div className="title" style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
                        <p className="title" >
                            <strong>Bağcılar, Türkiye</strong>
                        </p>
                        <p><StarFilled style={ {fontSize:"12px"}} /> 4,4</p>
                    </div>
                    <h5 style={{color:"#868686"}}>Geçen hafta 204 kez görüntülendi</h5>
                    <h5 style={{color:"#868686"}}>7 - 14 OCAK</h5>
                    <h5 style={{color:"#1d1d1d"}}><strong>5500TL</strong> Gece</h5>
                </div>
            </div>
            <div className="card">
                <div style={{ zIndex: "0", position: "absolute", marginLeft: "285px", marginTop: "10px" }}>
                    <HeartTwoTone twoToneColor="#eb2f96" style={{fontSize: "22px" }} />
                </div>
                <img src="https://a0.muscache.com/im/pictures/6943ce46-c64f-43b4-824b-5551735befcc.jpg?im_w=720" alt="" />
                <div className="info">
                    <div className="title" style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
                        <p className="title" >
                            <strong>Bağcılar, Türkiye</strong>
                        </p>
                        <p><StarFilled style={ {fontSize:"12px"}} /> 4,4</p>
                    </div>
                    <h5 style={{color:"#868686"}}>Geçen hafta 204 kez görüntülendi</h5>
                    <h5 style={{color:"#868686"}}>7 - 14 OCAK</h5>
                    <h5 style={{color:"#1d1d1d"}}><strong>5500TL</strong> Gece</h5>
                </div>
            </div>
            <div className="card">
                <div style={{ zIndex: "1", position: "absolute", marginLeft: "285px", marginTop: "10px" }}>
                    <HeartTwoTone twoToneColor="#eb2f96" style={{ fontSize: "22px" }} />
                </div>
                <img src="https://a0.muscache.com/im/pictures/6943ce46-c64f-43b4-824b-5551735befcc.jpg?im_w=720" alt="" />
                <div className="info">
                    <div className="title" style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
                        <p className="title" >
                            <strong>Bağcılar, Türkiye</strong>
                        </p>
                        <p><StarFilled style={ {fontSize:"12px"}} /> 4,4</p>
                    </div>
                    <h5 style={{color:"#868686"}}>Geçen hafta 204 kez görüntülendi</h5>
                    <h5 style={{color:"#868686"}}>7 - 14 OCAK</h5>
                    <h5 style={{color:"#1d1d1d"}}><strong>5500TL</strong> Gece</h5>
                </div>
            </div>
            
            
            
        </>
    )
}